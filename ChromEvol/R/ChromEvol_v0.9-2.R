...
#After analysis read the results obtained by fitting a specific model to the data with 
	###read.cece just by giving the path of the folder of the respective model
...
read.ancProb.tab <- function(filename){
	...
	anc.tab3 <- apply(anc.tab2[,2:length(anc.tab2[1,])], 2, as.numeric)
...
#############################################
collapse.clade.ce <- function(ChromEvol, node, name="collapsed", number=""){
